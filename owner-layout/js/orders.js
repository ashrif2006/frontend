...
const STATUS_LABELS = {
  ...
  CONFIRMED: { label: "مؤكد", badge: "badge-confirmed", cardClass: "confirmed" },
  SHIPPED: { label: "في الشحن", badge: "badge-shipped", cardClass: "shipped" },
  DELIVERED: { label: "تم التوصيل", badge: "badge-delivered", cardClass: "delivered" },
  CANCELLED: { label: "ملغي", badge: "badge-cancelled", cardClass: "cancelled" },
};
...
async function loadOrders() {
  ...
  allOrders = Array.isArray(result.data) ? result.data : result.data.orders || [];
  updateTabCounts();
  renderOrders();
}
...
function renderOrders() {
  ...
  filtered.forEach((order) => {
    ...
    card.querySelector(".order-card-header").addEventListener("click", () => toggleOrder(card));
    ...
    <div class="order-card ${statusInfo.cardClass}" data-status="${order.status}" data-order-id="${order.id}">
      ...
      <div class="order-card-body" style="display:none;">
        ...
        <div class="order-actions mt-2">
          ...
          ${order.customer_phone ? whatsappButtonHTML(order.customer_phone) : ""}
        </div>
      </div>
    </div>
  `;
...
function whatsappButtonHTML(phone) {
  const cleanPhone = phone.replace(/\D/g, "");
  return `
    <a href="https://wa.me/${cleanPhone}" target="_blank" class="btn btn-whatsapp me-auto">
